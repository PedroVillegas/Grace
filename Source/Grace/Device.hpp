...
public:
    ...
    GRACE_NODISCARD GpuHandle<T> Create(const GpuObjectDesc<T>& desc, GpuHandleFlags flags = GpuHandleFlags::None);

    template <GpuManaged T>
    GRACE_NODISCARD T& Get(const GpuHandle<T>& handle);

    template <GpuManaged T>
    void Free(GpuHandle<T>& handle);

    template <GpuManaged T>
    void FreeDeferred(GpuHandle<T>& handle);
...
private:
    ...
    std::unique_ptr<GpuObjectManager> mGpuObjManager = {};
